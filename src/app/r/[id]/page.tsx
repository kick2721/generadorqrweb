...
export default async function RedirectPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const rows = await query(`SELECT type, content, config FROM public.qrcodes WHERE id = $1`, [id]);
  ...
  if (qr.type === "email") {
    redirect(`/mail?to=${encodeURIComponent(content)}`);
  } else {
    redirect(content.startsWith("http") ? content : `https://${content}`);
  }
}
